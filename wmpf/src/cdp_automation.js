...
const discoverExecutionContextsById = async (client, maxContextId = 32, timeoutMs = 1500) => {
    const knownIds = new Set(client.getContexts().map((context) => context.id));
    const candidateIds = Array.from({ length: maxContextId }, (_, index) => index + 1)
        .filter((id) => !knownIds.has(id));
    const discovered = await Promise.all(candidateIds.map(async (contextId) => {
        try {
            const response = await client.sendCommand("Runtime.evaluate", {
                expression: `(() => ({
                    href: globalThis.location && typeof globalThis.location.href === "string"
            ...
            const href = response?.result?.value?.href ?? "";
            const context = {
                id: contextId,
                name: "",
                origin: href.includes("servicewechat.com") ? "https://servicewechat.com" : "",
                auxData: {
                    discoveredBy: "numeric_probe",
                },
            };
            if (typeof client.rememberContext === "function") {
                client.rememberContext(context);
            }
            return context;
        }
        catch (_) {
    ...
    return discovered.filter((context) => context !== null);
};
...
const selectExecutionContext = async (client, preferredContextName, logger, timeoutMs = CONTEXT_WAIT_TIMEOUT_MS) => {
    ...
    while (Date.now() - startedAt < timeoutMs) {
        ...
        if (client.getContexts().length === 0) {
            const discovered = await discoverExecutionContextsById(
                client,
                32,
                Math.min(1500, Math.max(250, remainingMs)),
            );
            if (discovered.length > 0) {
                logger.main_debug(`[auto] numerically discovered execution contexts: ${discovered.map((context) => context.id).join(", ")}`);
            }
        }
        try {
...
exports.discoverExecutionContextsById = discoverExecutionContextsById;
