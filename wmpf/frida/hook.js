...
const hookOnLoadScene = (a1, config) => {
    ...
    const [defaultBaseOffset, sceneRootOffset, sceneValueOffset] =
        config.ScenePathOffsets || [56, 8, 16];
    const baseOffset =
        typeof config.SceneBaseOffset === "number"
            ? config.SceneBaseOffset
            : defaultBaseOffset;

    const ptr1 = safeReadPointer(a1.add(baseOffset));
    if (!ptr1) {
        send(`[hook] scene chain: null at this+${baseOffset}`);
        return;
    }
    const ptr2 = safeReadPointer(ptr1.add(sceneOffsets[0]));
    if (!ptr2) {
        send(`[hook] scene chain: null at ptr1+${sceneOffsets[0]}`);
        return;
    }
    const ptr3 = safeReadPointer(ptr2.add(sceneRootOffset));
    if (!ptr3) {
        send(`[hook] scene chain: null at ptr2+${sceneRootOffset}`);
        return;
    }
    const ptr4 = safeReadPointer(ptr3.add(sceneOffsets[1]));
    if (!ptr4) {
        send(`[hook] scene chain: null at ptr3+${sceneOffsets[1]}`);
        return;
    }
    const ptr5 = safeReadPointer(ptr4.add(sceneValueOffset));
    if (!ptr5) {
        send(`[hook] scene chain: null at ptr4+${sceneValueOffset}`);
        return;
    }
    const miniappScenePtr = ptr5.add(sceneOffsets[2]);
    const sceneRange = Process.findRangeByAddress(miniappScenePtr);
    if (!sceneRange || !sceneRange.protection.includes("r")) {
