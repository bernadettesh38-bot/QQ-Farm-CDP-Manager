...
- v2.9.4 improves immediate daily actions, own-farm harvest batching,
  multi-account settings selection, and desktop control reliability.
- v2.9.4 automatically handles the native QQ account chooser for configured
  additional accounts and adds exact WMPF20079 compatibility with safe
  exact-profile discovery for future WeChat runtime builds.
- v2.9.4 refreshes source/protected regression, integrity, and performance
  verification for normal Windows startup.
...
- v2.9.2 improves current WeChat runtime discovery and cold-start context
  recovery while retaining legacy WMPF compatibility.
- v2.9.2 hardens long-running memory, diagnostics, LAN control, reconnect, and
  scheduled-recovery behavior without removing farm automation features.
- v2.9.2 refreshes source/protected verification for normal Windows startup,
  signed-package integrity, runtime contracts, and performance safeguards.
...
- v2.5.5 adds UTC+8 scheduled warehouse item use for configured tool items, including same-flow support for normal fertilizer, organic fertilizer, and package-style item popups.
- v2.5.5 rechecks other-place login reconnect handling and keeps protected/lite runtime size budgets green after the warehouse and WeChat fixes.
